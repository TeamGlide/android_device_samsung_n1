...
KXTFSensor::~KXTFSensor()
{
    ALOGV(LOG_TAG "[KXTF9] " __FUNCTION__);
    if (mEnabled) {
        enable(0, 0);
    }
}
...
int KXTFSensor::setInitialState()
{
    ...
    if (mHasPendingEvent) {
        ALOGD(LOG_TAG "[KXTF9] %s: %d %d %d -> %f %f %f", __FUNCTION__,
                absinfo_x.value, absinfo_y.value, absinfo_z.value,
...
int KXTFSensor::enable(int32_t, int en)
{
    ...
    if (flags != mEnabled) {
        ...
        ALOGI(LOG_TAG "[KXTF9] %s(%d) %s", __FUNCTION__, en, input_sysfs_path);
        ...
        ALOGE(LOG_TAG "[KXTF9] %s(%d) failed, err %d", __FUNCTION__, en, errno);
        return -1;
    }
    return 0;
}
...
again:
    ...
    while (count && mInputReader.readEvent(&event)) {
        ...
        if (type == EV_REL) {
            ...
            ALOGE(LOG_TAG "[KXTF9] %s: unknown event (type=%d, code=%d)",
    ...
    if (mHasPendingEvent) {
        ALOGV(LOG_TAG "[KXTF9] %s: %d %d %d -> %f %f %f", __FUNCTION__,
                absinfo_x.value, absinfo_y.value, absinfo_z.value,
                mPendingEvent.data[0], mPendingEvent.data[1], mPendingEvent.data[2]);
    }
